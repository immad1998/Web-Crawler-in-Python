	...
	while True:
		...
		if (data == exitCMD): # If the client sends a 'bye' then release the lock for the next client in queue and close this current connection
			...
			for x in file_to_set(CRAWLED_FILE):
				links += x 
				links += '\n'
			conn.sendall(links.encode()) #send all the links to client

		if not data: # Release the lock if no data is received from client and close the connection by breaking the loop
			lock.release()
			break;
